...
import logging
from app.models.communication_event_purpose_types import create_communication_event_purpose_type, get_communication_event_purpose_type, update_communication_event_purpose_type, delete_communication_event_purpose_type, get_all_communication_event_purpose_types
...
router = APIRouter(prefix="/communication_event_purpose_types", tags=["communication_event_purpose_types"])

oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/auth/login")

# สร้าง communication_event_purpose_type ใหม่ (basetype_admin เท่านั้น)
...
async def create_communication_event_purpose_type_endpoint(communication_event_purpose_type: CommunicationEventPurposeTypeCreate, current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "basetype_admin":
        logger.warning(f"Unauthorized attempt to create communication_event_purpose_type by user: id={current_user['id']}, role={current_user['role']}")
    ...
    if not result:
        logger.warning(f"Failed to create communication_event_purpose_type: {communication_event_purpose_type.description}")
...
async def get_communication_event_purpose_type_endpoint(communication_event_purpose_type_id: int, current_user: dict = Depends(get_current_user)):
    ...
    if not result:
        ...
        raise HTTPException(status_code=404, detail="Communication event purpose type not found")
...
# อัปเดตข้อมูล communication_event_purpose_type (basetype_admin เท่านั้น)
...
async def update_communication_event_purpose_type_endpoint(communication_event_purpose_type_id: int, communication_event_purpose_type: CommunicationEventPurposeTypeUpdate, current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "basetype_admin":
        logger.warning(f"Unauthorized attempt to update communication_event_purpose_type by id={communication_event_purpose_type_id} by user: id={current_user['id']}, role={current_user['role']}")
        raise HTTPException(status_code=403, detail="Basetype admin access required")
...
# ลบ communication_event_purpose_type (basetype_admin เท่านั้น)
...
async def delete_communication_event_purpose_type_endpoint(communication_event_purpose_type_id: int, current_user: dict = Depends(get_current_user)):
    if current_user["role"] != "basetype_admin":
        logger.warning(f"Unauthorized attempt to delete communication_event_purpose_type by id={communication_event_purpose_type_id} by user: id={current_user['id']}, role={current_user['role']}")
        raise HTTPException(status_code=403, detail="Basetype admin access required")
