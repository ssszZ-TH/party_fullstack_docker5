...
// Services data for basetype_admin
const services = [
  {
    ...
    items: [
      ...
      { name: "Communication Event Purpose Type", path: "/v1/communicationeventpurposetype" },
      { name: "Contact Mechanism Type", path: "/v1/contactmechanismtype" },
      { name: "Communication Event Status Type", path: "/v1/communicationeventstatustype" },
      { name: "Priority Type", path: "/v1/prioritytype" },
      { name: "Party Relationship Status Type", path: "/v1/partyrelationshipstatustype" },
...
// Navigation items
const navItems = [
  { name: "Profile", icon: <PersonIcon />, path: "/profiles/basetype_admin" }, // อัปเดต path ไปยังหน้า profile ใหม่
...
export default function BasetypeAdminHome() {
  ...
  useEffect(() => {
    const checkTokenValidity = async () => {
      ...
      if (!isAuthenticated || !token || role !== "basetype_admin") {
        logout();
