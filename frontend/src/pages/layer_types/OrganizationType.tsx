...
export default function OrganizationTypes() {
    ...
    const fetchTypes = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          setTypes(data as OrganizationType[]);
          setError(null);
        } else {
  ...
  const columns: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        params.id ? (
          <UpdateButton
            onClick={() => navigate(`/organization-type/${params.id}`)}
          />
        ) : null
      ),
    },
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', display: 'flex', flexDirection: 'column' }}>
      <AppBarCustom title="Organization Types" />
      <Container maxWidth="lg" sx={{ flexGrow: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h4" sx={{ color: 'text.primary' }}>
            All Organization Types
          </Typography>
          <AddButton onClick={() => navigate('/organization-type/create')} />
        </Box>
        {loading ? (
          <Loading />
        ) : error ? (
